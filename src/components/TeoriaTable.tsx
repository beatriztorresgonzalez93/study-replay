  ...
  return (
    <div className="glass rounded-2xl border-amber-500/15 p-3 md:p-4">
      <div className="mb-4 flex items-center justify-between gap-3 px-1">
        ...
        <div className="h-1.5 flex-1 max-w-[8rem] overflow-hidden rounded-full bg-black/40">
          <div
                ...
                {Array.from({ length: TOPIC_COUNT }, (_, ti) => {
                  ...
                  return (
                    <td key={ti} className="text-center">
                      <label
                        className={`teoria-check inline-flex cursor-pointer items-center justify-center rounded-lg border p-2.5 transition ${
                          checked
                      ...
                      >
                        <input
                          ...
                          checked={checked}
                          disabled={loading}
                          onChange={(e) => toggle(si, ti, e.target.checked)}
                          aria-label={`${initial.subjectNames[si]} ${DEFAULT_TOPIC_NAMES[ti]}${checked ? ", completado" : ""}`}
                        />
                        <span
